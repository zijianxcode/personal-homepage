...
首页新增按时间回溯入口，需要支持 `最新一天`、`近 3 天`、`近 7 天`、`本月` 等快捷筛选。

### 风险判断

静态站点不一定每天重新部署。如果按浏览器当天计算时间范围，站点几天未更新时，`最新一天` 很可能返回空列表，用户会误以为资料丢失。

### 修复动作

- 归档页读取所有记录的 `entry.date`，先找出数据中的最新记录日期。
- `latest-day`、`3d`、`7d` 和 `month` 都以这个最新记录日期为基准过滤。
- `search` 和 `dateRange` 同时存在时同时生效。
- 首页只放快捷入口，不直接渲染结果，避免首页信息密度再次失控。

### 预防措施

- 后续新增时间类筛选时，默认以数据时间为基准，只有实时服务页面才考虑浏览器当前时间。
- 发布时必须同步 `site-detail.js`、`site-i18n.js` 和更新版本号后的 HTML。
- 新增 URL 参数后，需要测试“参数单独生效”和“与搜索参数组合生效”两种路径。

## 2026-05-15：英文切换后仍有部分 UI 文案未翻译

### 问题现象

- 切换到 EN 后，首页研究方向说明、排行榜奖牌、未评分说明和详情页段落导航仍出现中文。
- 这些文本不是全部来自 HTML 初始模板，有些来自动态渲染或从正文标题复制出的目录标签。

### 根因判断

`site-i18n.js` 的词典覆盖了主要导航和按钮，但没有覆盖：

1. 研究方向卡片里的说明文本。
2. 排行榜奖牌和未评分理由。
3. 详情页 `readerToc` 从正文标题复制出来的常见中文章节名。
4. 高星页里的角色评分标签。

### 修复动作

- 补充 `site-i18n.js` 的精确词典和模式匹配。
- 对首页、排行榜页、论文页、归档页、高星页做浏览器扫描，切 EN 后排除语言按钮 `中`，确认没有剩余中文 UI 文案。

### 预防措施

- 新增 UI 文案时，如果不是论文正文或论文标题，必须同步补充 `site-i18n.js`。
- 详情页目录虽然来自正文标题，但属于导航 UI；常见章节名需要加入英文映射。

## 2026-05-15：全局语言切换不能只改 HTML，必须由生成脚本和共享 JS 承担

### 问题背景

`academy` 需要增加中英文切换，默认中文，右上角切换英文，并且是全局能力。

### 实现判断

这个功能不能直接手改 `index.html`，否则下一次 `sync_from_source.py` 生成页面时会被覆盖。正确位置是：

1. `sync_from_source.py`：给首页、排行榜页和详情页统一注入语言切换入口。
2. `site-i18n.js`：负责切换、保存偏好、翻译动态渲染出来的 UI 文案。
3. `site.css`：负责右上角切换控件样式和英文态搜索框宽度/字号适配。

### 范围边界

- 翻译站点外壳：导航、按钮、入口说明、统计标签、排行榜状态、详情页工具栏和空状态。
- 不自动翻译论文正文、论文标题和记录标题，避免机器替换破坏研究语义。

### 避坑说明

- 发布 CloudBase/`academy` 时，除了 HTML 和 `site.css`，还必须同步 `site-i18n.js`。
- 若后续新增页面模板，必须同时引入语言切换入口和 `site-i18n.js`。
- 动态页面文案如果由 `site-detail.js` 新增，需要同步补充 `site-i18n.js` 的词典或模式匹配。
